...
# global variables
hash_list = [] #contain all hash value for all pages
...
#extra credit 2 functions (start)

# input: soup object generated by Beautiful soup
def get_fingerprint(soup, n=3):
    ...
    hash_set = {hash(k) for k in counts.keys() if hash(k) % 4 == 0}

    return hash_set

#this function takes in the hash_set for two page and check if they are too similar
def checkif_toosimilar(left, right, threshold = 0.7):
    #commpute intersection and union, since we not allowed to use .intersect()
    intersection = set([v for v in left if v in right])
    union = left.copy()
    union.update([v for v in right if v not in left])
    
    #compute the score, 0 if there's no union, since can't divide by 0
...
def extract_next_links(url, resp):
    ...
    FA = get_fingerprint(soup)
    global hash_list
    is_similar = any(checkif_toosimilar(FA, v) for v in hash_list)
